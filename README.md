...
_Add screenshots of the chat interface after deployment. Include one screenshot that clearly shows the active persona switcher._
...
Edit `.env.local` and add your Gemini API key and backend URL:

```
GEMINI_API_KEY=your_gemini_api_key_here
NEXT_PUBLIC_BACKEND_URL=https://genaibot-hvcc.onrender.com
```

Get your API key from [aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey). If `NEXT_PUBLIC_BACKEND_URL` is not set, the frontend falls back to the local Next.js route at `/api/chat`.
...
│   └── page.tsx
├── components/
│   ├── ChatSession.tsx       # Main chat component (state + logic)
│   ├── MentorSelector.tsx    # Tab switcher for personas
│   ├── MessageEntry.tsx      # Individual chat message
│   └── StatusIndicator.tsx   # Animated typing indicator
├── lib/
│   └── mentors.ts            # Persona definitions and system prompts
